...
## ステップ 5: 匿名認証（Anonymous Auth）の有効化
1. 左メニューの **「構築」>「Authentication」** をクリックします。
2. **「Sign-in method」** タブを開きます。
3. **「匿名 (Anonymous)」** を見つけてクリックし、ステータスを **「有効」** にして保存します。

## ステップ 6: 本番用セキュリティルールの適用
**重要:** 公開前に必ずこの設定を行ってください。

1. 左メニューの **「Firestore Database」** に戻ります。
2. **「ルール (Rules)」** タブをクリックします。
3. エディタの内容を以下のように書き換えて、「公開」をクリックします。

```
rules_version = '2';
service cloud.firestore {
  match /databases/{database}/documents {
    match /appData/presets {
